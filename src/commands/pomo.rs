...
//    ctx: Context<'_>,
//    #[description = "Duration of a work session in minutes (default: 25)"]
// work: Option<usize>,    #[description = "Duration of a short break in minutes
// (default: 5)"] short: Option<usize>,    #[description = "Duration of a long
// break in minutes (default: 15)"] long: Option<usize>,    #[description =
// "Number of work sessions to run (default: 8)"] sessions: Option<usize>,
//    #[description = "How many work sessions between each long break (default:
// 4)"] interval: Option<        usize,
//    >,
//) -> Result<(), Error> {
...
//                There is already an active session in this channel! Please run
// the `stop` command to stop it.                ",
//            })
//        })
//        .await?;
...
//            Working for {sessions} sessions of {work} minutes each, with a
// {short} minute short break after each one, and a {long} minute long break
// after every {interval} work sessions.            ",
//            sessions = sessions,
//            work = work,
//            short = short,
//            long = long,
...
//                        Finished work session #{session_num}! Starting a
// {long} minute long break!                        ",
...
//                    .send_message(&ctx.discord().http, |msg| {
//                        msg.content(formatdoc! { "
//                            You made it through {sessions} work sessions! Nice
// job!                            ",
//                            sessions = sessions,
//                        })
//                        .tts(true)
...
//        poise::send_reply(ctx, |reply| {
//            reply.content(formatdoc! { "
//                There is no active session in this channel! Please run the
// `start` command to start one.                ",
//            })
//        })
//        .await?;
//    } else {
...
///// `ctx.channel_id()`.
/////
///// If no session is running and `set_running` is `true`, then the value will
///// be changed to `true`.
